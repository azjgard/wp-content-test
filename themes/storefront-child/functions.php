...
function add_show_more_products_message() {
  echo "<div class='product-archive-message'>
          (Adjust filter options to view more products)
        </div>";
}
add_action( 'woocommerce_after_shop_loop', 'add_show_more_products_message' );
...
// This is the function that handles all of the filtering.
function get_filter_exclusions($category_slug = false, $filter_args = false)
{
    ...
    global_debug( $attributes, "Attributes beginning" );

    $unfiltered_products = maybe_unserialize(
        get_option($category_slug . '-products-object')
    );

    global_debug( $unfiltered_products, "Unfiltered products" );
    ...
    if ($filter_args) {
        for ($i = 0; $i < sizeof($unfiltered_products); $i++) {
            ...
            foreach ($filter_args as $name => $filter_meta_value) {
                ...
                global_debug( $meta, "PMM" );
                global_debug( slugify($name), "slugified");

                /* global_debug( $product_meta_value, "PM" ); */
            ...
            if ($product_does_match) {
                ...
                foreach ($product->meta as $attribute_object) {
                  unset($attributes[$attribute_object['name']][normalize($attribute_object['value'])]);
                }
    ...
    global_debug( $attributes, "Attributes end" );
...
$attribute_data = array(
    ...
    'GC Septa' => array(
        ...
        // Temporarily taking this out; there's a billion commas in each value, 
        // and that causes it to import each comma-separated value as a
        // separate option. This is fine, but it makes things hecka tricky
        // when trying to think about comparing. There will also be half
        // a billion filter options. I'd have to add support specially
        // checking for when values contain commas, and then check for each
        // one..
        /* array( */
...
foreach ($categories as $category) {
            ...
            $subcategories = $this->attribute_data[$name];

            echo '<option value="' . $slug . '">' . $name . '</option>';

            $filter_html .= '<div class="' . $slug . '-attribute-filter no-display">';

            foreach ($subcategories as $subcategory) {

                $subcat_name = $subcategory['subcategory_name'];
                $subcat_attr = $subcategory['subcategory_attr'];

                $filter_html .= '<h3 class="subcategory-title" name="' . $slug . '">' . $subcat_name . '</h3>';
                $filter_html .= '<div class="subcategory" name="' . $subcat_name . '">';

                foreach ($subcat_attr as $attr) {
                    $filter_html .= '<div><input type="checkbox" name="' . $subcat_name . '" /><label>' . $attr . '</label></div>';
                }
